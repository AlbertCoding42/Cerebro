...
def plot_data_3D(X, color, title, exp_name=None, filename=None, axes=False, node_size=0.5, opacity=1, cmap=None, labels=None, camera=None, colorbar=False, text=None):
    ...
    if labels is not None:
        ...
        fig.add_trace(go.Scatter3d(
            ...
            marker=dict(
                ...
                opacity=opacity,
                colorbar=dict(title="Color Scale") if colorbar is True else None,  # Show colorbar for continuous colormap
            ),
            showlegend=False,
            text=text,
            hoverinfo='text',
